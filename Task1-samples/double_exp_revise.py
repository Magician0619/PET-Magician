...
rate = 1
x_dexp = x/rate
x_dexp = np.linspace(0,x_dexp[7],1000)
y_dexp = poly_func(x_dexp)
...
y_inter = double_exp(x_dexp,popt[0],popt[1],popt[2],popt[3],popt[4],popt[5]) # 拟合y值

plt.figure("双指数插值曲线")
# plt.plot(x, y, 's',label='original values')
plt.plot(x_dexp, y_inter, 'r',label='polyfit values')
